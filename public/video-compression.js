document.addEventListener("DOMContentLoaded", function () {
  ...
  videoInput.addEventListener("change", function () {
    if (this.files && this.files[0]) {
      ...
      // --- Changed Code Starts ---
      // Instead of using FileReader to convert the file to a data URL,
      // directly create a blob URL for the uploaded video.
      // This allows large videos to load instantly without memory issues.
      const fileURL = URL.createObjectURL(file);
      uploadedVideo.src = fileURL;
      uploadedVideo.load();
      uploadedVideo.onloadedmetadata = function () {
        const originalWidth = uploadedVideo.videoWidth;
        const originalHeight = uploadedVideo.videoHeight;
